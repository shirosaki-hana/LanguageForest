import { z } from 'zod';
import type { TemplateParser, HelperRegistry } from './templateParser.js';

/**
 * 기본 템플릿 헬퍼들을 파서에 등록합니다.
 * 문자열 변환, 비교/논리 연산, 배열/객체 유틸 등을 제공합니다.
 */
export function registerDefaultHelpers(parser: TemplateParser): void {
  const helpers: HelperRegistry = {
    ...
    // 비교/논리 연산 (서브표현식으로 사용: {{#if (eq a b)}})
    eq(this: unknown, a: unknown, b: unknown) {
      // eq/ne는 엄격 동등 비교 semantics 유지
      return a === b;
    },
    ...
    and(this: unknown, ...args: unknown[]) {
      const values = args.slice(0, -1); // 마지막 인자는 Handlebars options
